...
A single line will be printed in stdout, the seed of the current game in the specified map
'''
...
def copy_map_saved_data(source_json, target_json, map_name, output_json):
    ...
    #print seed
    print(target_data['savedMaps'][map_name]['freeplayRoundSeed'])
    
    with open(output_json, 'w') as f:
        json.dump(target_data, f, indent=4)
    
    eprint(f"Copied savedMaps for '{map_name}' to {output_json}")
...
def extract_tower_pops(json_file: str, map_name: str, output_csv:str):
    ...
    try:
        ...
        if map_name not in data['savedMaps']:
            eprint(f"Error: Map '{map_name}' not found in save file")
            eprint(f"Available maps: {', '.join(data['savedMaps'].keys())}")
            sys.exit(1)
            
        if 'placedTowers' not in data['savedMaps'][map_name]:
            eprint(f"Error: No placed towers found in map '{map_name}'")
            sys.exit(1)
        ...
        eprint(f"Successfully wrote data to {output_csv}")
        eprint(f"Found {len(csv_data)} different tower types with a total of {sum(item['count'] for item in tower_pops.values())} towers")
        
    except FileNotFoundError:
        eprint(f"Error: File '{json_file}' not found")
        sys.exit(1)
    except json.JSONDecodeError:
        eprint(f"Error: '{json_file}' is not a valid JSON file")
        sys.exit(1)
...
def pack_json(source_json, output_save, monke_exe):
    process = subprocess.Popen([monke_exe, 'pack', source_json, output_save, '11'], stdout = subprocess.DEVNULL)
    process.wait()

def unpack_json(source_save, output_json, monke_exe):
    process = subprocess.Popen([monke_exe, 'unpack', source_save, output_json, '11'], stdout = subprocess.DEVNULL)
    process.wait()

def main():
    if len(sys.argv) < 3 or len(sys.argv) > 5:
        eprint("Usage: python script.py <input_file> <map_name> [<output_file>] [<popcount_csv>]")
        sys.exit(1)
    ...
    try:
        ...
        if export_save:
            ...
            eprint(f"Successfully processed {map_name} and created {output_file}")
